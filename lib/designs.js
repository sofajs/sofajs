...
exports = module.exports = internals.Tools = function (sofaInternalsParam) {
    ...
    sofaInternals.design.loadold = function () {
    ...
    sofaInternals.design.load = function (done) {
        ...
        // sofaInternals.utils.core.processDesign(fixtures, designDocument, done);

        //     // Async.waterfall([function (next) {
        //     //     sofaInternals.utils.core.hashem2(internals.designFixtures[0], next);
        //     // }], function (err) {
        //     //     console.log('done after hashem2: ' + typeof(done));
        //     //     done();
        //     // });
        // }  else {
        // 
        ...
        Async.waterfall([function (next) {
            ...
            if (designName === 'user') {
                ...
                for (var i = 0; i < fixtures.length; ++i) {
                    // console.log(designName + 'count: ' + i ); 
                    // console.log('original pw: ' + fixtures[i].pw);
            ...
            sofaInternals.utils.core.findById('_design/' + designName, function (err, result) {

                // design loaded

                console.log('** ' + JSON.stringify(result._id) + ' loaded **');
                console.log('Design name: "' + designName + '" _designs & fixtures loaded');
                console.log('completed design load!!  done() ' + typeof(done));
            });
        });
    };
    ...
    this.loadDesigns = function (callback) {
        ...
        Async.each(designs, function(design, callback) { 
            ...
            console.log('design pre execution: ' + design);
            design.call(sofaInternals, sofaInternals, callback);
            // callback();
            //design.call(sofaInternals, sofaInternals);
        }, function (err) {

            console.log('Async.each() completed: ' + err);
        });
