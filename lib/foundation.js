...
internals.loadRequest = function (request) {
    ...
    // @todo make load request a foundation or base request.
    // request process
    // * connect
    // * execute request
    ...
    // enforce foundation param count logic begins here.
    // inpect number of arguments in handler function to be built.

    var paramCount = internals.countParams(request.handler);

    Hoek.assert(paramCount <= 3 , 'foundation.' + request.group + '.' + request.name + '() has too many parameters.');
